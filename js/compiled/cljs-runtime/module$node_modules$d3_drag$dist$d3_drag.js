...
gestures[touches[i].identifier])event.stopImmediatePropagation(),gesture("end",event,touches[i])}function beforestart(that,container,event$jscomp$0,d,identifier,touch){var dispatch=listeners.copy(),p=d3Selection.pointer(touch||event$jscomp$0,container),s;if(null!=(s=subject.call(that,new DragEvent("beforestart",{sourceEvent:event$jscomp$0,target:drag,identifier,active,x:p[0],y:p[1],dx:0,dy:0,dispatch}),d))){var dx=s.x-p[0]||0;var dy=s.y-p[1]||0;return function gesture(type,event,touch){var p0=p;switch(type){case "start":gestures[identifier]=
gesture;var n=active++;break;case "end":delete gestures[identifier],--active;case "drag":p=d3Selection.pointer(touch||event,container),n=active}dispatch.call(type,that,new DragEvent(type,{sourceEvent:event,subject:s,target:drag,identifier,active:n,x:p[0]+dx,y:p[1]+dy,dx:p[0]-p0[0],dy:p[1]-p0[1],dispatch}),d)}}}var filter=defaultFilter,container$jscomp$0=defaultContainer,subject=defaultSubject,touchable=defaultTouchable,gestures={},listeners=d3Dispatch.dispatch("start","drag","end"),active=0,mousedownx,
mousedowny,mousemoving,touchending,clickDistance2=0;drag.filter=function(_){return arguments.length?(filter="function"===typeof _?_:constant(!!_),drag):filter};drag.container=function(_){return arguments.length?(container$jscomp$0="function"===typeof _?_:constant(_),drag):container$jscomp$0};drag.subject=function(_){return arguments.length?(subject="function"===typeof _?_:constant(_),drag):subject};drag.touchable=function(_){return arguments.length?(touchable="function"===typeof _?_:constant(!!_),

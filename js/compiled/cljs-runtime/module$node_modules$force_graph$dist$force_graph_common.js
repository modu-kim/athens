...
_ref4.type;_ref4.objs.filter(function(d){if(!d.hasOwnProperty("__indexColor"))return!0;var cur=state.colorTracker.lookup(d.__indexColor);return!cur||!cur.hasOwnProperty("d")||cur.d!==d}).forEach(function(d){d.__indexColor=state.colorTracker.register({type:type,d:d})})});state.forceGraph.graphData(d$jscomp$0);state.shadowGraph.graphData(d$jscomp$0)},triggerUpdate:!1},backgroundColor:{onChange:function(color,state){state.canvas&&color&&(state.canvas.style.background=color)},triggerUpdate:!1},nodeLabel:{"default":"name",
...
state.linkHoverPrecision});var t=d3Zoom.zoomTransform(state.canvas);state.shadowGraph.globalScale(t.k).tickFrame()},800);(this._animationCycle=function animate(){if(state.enablePointerInteraction){var obj=null;if(!state.isPointerDragging){var pxScale=window.devicePixelRatio;(pxScale=0<JSCompiler_object_inline_x_1758&&0<JSCompiler_object_inline_y_1759?shadowCtx.getImageData(JSCompiler_object_inline_x_1758*pxScale,JSCompiler_object_inline_y_1759*pxScale,1,1):null)&&(obj=state.colorTracker.lookup(pxScale.data))}if(obj!==

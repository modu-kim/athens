...
x)?hi=mid:lo=mid+1}return lo}}}function ascendingComparator(f){return function(d,x){return ascending(f(d),x)}}function number(x){return null===x?NaN:+x}function numbers(values,valueof){var index$80,$jscomp$iter$4,$jscomp$key$value,value$81,$jscomp$iter$3,value;return $jscomp.generator.createGenerator(numbers,function($jscomp$generator$context){switch($jscomp$generator$context.nextAddress){case 1:if(void 0===valueof){$jscomp$generator$context.jumpTo(2);break}index$80=-1;$jscomp$iter$4=$jscomp.makeIterator(values);
$jscomp$key$value=$jscomp$iter$4.next();case 4:if($jscomp$key$value.done){$jscomp$generator$context.jumpTo(0);break}value$81=$jscomp$key$value.value;if(!(null!=(value$81=valueof(value$81,++index$80,values))&&(value$81=+value$81)>=value$81)){$jscomp$generator$context.jumpTo(5);break}return $jscomp$generator$context.yield(value$81,5);case 5:$jscomp$key$value=$jscomp$iter$4.next();$jscomp$generator$context.jumpTo(4);break;case 2:$jscomp$iter$3=$jscomp.makeIterator(values),$jscomp$key$value=$jscomp$iter$3.next();
case 9:if($jscomp$key$value.done){$jscomp$generator$context.jumpTo(0);break}value=$jscomp$key$value.value;if(!(null!=value&&(value=+value)>=value)){$jscomp$generator$context.jumpTo(10);break}return $jscomp$generator$context.yield(value,10);case 10:$jscomp$key$value=$jscomp$iter$3.next(),$jscomp$generator$context.jumpTo(9)}})}function count$jscomp$0(values,valueof){var count=0;if(void 0===valueof){values=$jscomp.makeIterator(values);for(var $jscomp$key$value=values.next();!$jscomp$key$value.done;$jscomp$key$value=
values.next())valueof=$jscomp$key$value.value,null!=valueof&&(valueof=+valueof)>=valueof&&++count}else{var index$82=-1,$jscomp$iter$6=$jscomp.makeIterator(values);for($jscomp$key$value=$jscomp$iter$6.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$6.next())$jscomp$key$value=$jscomp$key$value.value,null!=($jscomp$key$value=valueof($jscomp$key$value,++index$82,values))&&($jscomp$key$value=+$jscomp$key$value)>=$jscomp$key$value&&++count}return count}function length$jscomp$0(array){return array.length|
...
(delta=valueof-mean,mean+=delta/++count,sum+=delta*(valueof-mean));else{var index$87=-1,$jscomp$iter$8=$jscomp.makeIterator(values);for(delta=$jscomp$iter$8.next();!delta.done;delta=$jscomp$iter$8.next()){var value$88=delta.value;null!=(value$88=valueof(value$88,++index$87,values))&&(value$88=+value$88)>=value$88&&(delta=value$88-mean,mean+=delta/++count,sum+=delta*(value$88-mean))}}if(1<count)return sum/(count-1)}function deviation(values,valueof){return(values=variance(values,valueof))?Math.sqrt(values):
values}function extent(values,valueof){var min,max;if(void 0===valueof){values=$jscomp.makeIterator(values);for(var $jscomp$key$value=values.next();!$jscomp$key$value.done;$jscomp$key$value=values.next())valueof=$jscomp$key$value.value,null!=valueof&&(void 0===min?valueof>=valueof&&(min=max=valueof):(min>valueof&&(min=valueof),max<valueof&&(max=valueof)))}else{var index$89=-1,$jscomp$iter$10=$jscomp.makeIterator(values);for($jscomp$key$value=$jscomp$iter$10.next();!$jscomp$key$value.done;$jscomp$key$value=
$jscomp$iter$10.next())$jscomp$key$value=$jscomp$key$value.value,null!=($jscomp$key$value=valueof($jscomp$key$value,++index$89,values))&&(void 0===min?$jscomp$key$value>=$jscomp$key$value&&(min=max=$jscomp$key$value):(min>$jscomp$key$value&&(min=$jscomp$key$value),max<$jscomp$key$value&&(max=$jscomp$key$value)))}return[min,max]}function intern_get($jscomp$destructuring$var8,value){var _intern=$jscomp$destructuring$var8._intern;$jscomp$destructuring$var8=$jscomp$destructuring$var8._key;$jscomp$destructuring$var8=
$jscomp$destructuring$var8(value);return _intern.has($jscomp$destructuring$var8)?_intern.get($jscomp$destructuring$var8):value}function intern_set($jscomp$destructuring$var10,value){var _intern=$jscomp$destructuring$var10._intern;$jscomp$destructuring$var10=$jscomp$destructuring$var10._key;$jscomp$destructuring$var10=$jscomp$destructuring$var10(value);if(_intern.has($jscomp$destructuring$var10))return _intern.get($jscomp$destructuring$var10);_intern.set($jscomp$destructuring$var10,value);return value}
function intern_delete($jscomp$destructuring$var12,value){var _intern=$jscomp$destructuring$var12._intern;$jscomp$destructuring$var12=$jscomp$destructuring$var12._key;$jscomp$destructuring$var12=$jscomp$destructuring$var12(value);_intern.has($jscomp$destructuring$var12)&&(value=_intern.get(value),_intern.delete($jscomp$destructuring$var12));return value}function keyof(value){return null!==value&&"object"===typeof value?value.valueOf():value}function identity(x){return x}function group(values,keys){for(var $jscomp$restParams=
...
return values[0]}function nest(values$jscomp$0,map,reduce,keys){return function regroup(values,i){if(i>=keys.length)return reduce(values);for(var groups=new InternMap,keyof=keys[i++],index=-1,$jscomp$iter$15=$jscomp.makeIterator(values),$jscomp$key$value=$jscomp$iter$15.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$15.next()){$jscomp$key$value=$jscomp$key$value.value;var key=keyof($jscomp$key$value,++index,values),group$100=groups.get(key);group$100?group$100.push($jscomp$key$value):
...
1]=arguments[$jscomp$restIndex];var F$103=$jscomp$restParams;if("function"!==typeof values[Symbol.iterator])throw new TypeError("values is not iterable");values=Array.from(values);$jscomp$restParams=$jscomp.makeIterator(F$103).next().value;var f$jscomp$0=void 0===$jscomp$restParams?ascending:$jscomp$restParams;return 1===f$jscomp$0.length||1<F$103.length?($jscomp$restParams=Uint32Array.from(values,function(d,i){return i}),1<F$103.length?(F$103=F$103.map(function(f){return values.map(f)}),$jscomp$restParams.sort(function(i,
j){for(var $jscomp$iter$17=$jscomp.makeIterator(F$103),$jscomp$key$f=$jscomp$iter$17.next();!$jscomp$key$f.done;$jscomp$key$f=$jscomp$iter$17.next())if($jscomp$key$f=$jscomp$key$f.value,$jscomp$key$f=ascending($jscomp$key$f[i],$jscomp$key$f[j]))return $jscomp$key$f})):(f$jscomp$0=values.map(f$jscomp$0),$jscomp$restParams.sort(function(i,j){return ascending(f$jscomp$0[i],f$jscomp$0[j])})),permute(values,$jscomp$restParams)):values.sort(f$jscomp$0)}function constant(x){return function(){return x}}function ticks(start,
...
function max$jscomp$0(values,valueof){var max;if(void 0===valueof){values=$jscomp.makeIterator(values);for(var $jscomp$key$value=values.next();!$jscomp$key$value.done;$jscomp$key$value=values.next())valueof=$jscomp$key$value.value,null!=valueof&&(max<valueof||void 0===max&&valueof>=valueof)&&(max=valueof)}else{var index$107=-1,$jscomp$iter$19=$jscomp.makeIterator(values);for($jscomp$key$value=$jscomp$iter$19.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$19.next())$jscomp$key$value=
$jscomp$key$value.value,null!=($jscomp$key$value=valueof($jscomp$key$value,++index$107,values))&&(max<$jscomp$key$value||void 0===max&&$jscomp$key$value>=$jscomp$key$value)&&(max=$jscomp$key$value)}return max}function min$jscomp$0(values,valueof){var min;if(void 0===valueof){values=$jscomp.makeIterator(values);for(var $jscomp$key$value=values.next();!$jscomp$key$value.done;$jscomp$key$value=values.next())valueof=$jscomp$key$value.value,null!=valueof&&(min>valueof||void 0===min&&valueof>=valueof)&&
(min=valueof)}else{var index$109=-1,$jscomp$iter$21=$jscomp.makeIterator(values);for($jscomp$key$value=$jscomp$iter$21.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$21.next())$jscomp$key$value=$jscomp$key$value.value,null!=($jscomp$key$value=valueof($jscomp$key$value,++index$109,values))&&(min>$jscomp$key$value||void 0===min&&$jscomp$key$value>=$jscomp$key$value)&&(min=$jscomp$key$value)}return min}function quickselect(array,k,left,right,compare){left=void 0===left?0:left;right=void 0===
...
1));values=min$jscomp$0(values.subarray(valueof+1));return value0+(values-value0)*(p-valueof)}}function maxIndex$jscomp$0(values,valueof){var maxIndex=-1,index=-1;if(void 0===valueof){values=$jscomp.makeIterator(values);for(var $jscomp$key$value=values.next();!$jscomp$key$value.done;$jscomp$key$value=values.next())if(valueof=$jscomp$key$value.value,++index,null!=valueof&&(max<valueof||void 0===max&&valueof>=valueof)){var max=valueof;maxIndex=index}}else{var $jscomp$iter$23=$jscomp.makeIterator(values);
for($jscomp$key$value=$jscomp$iter$23.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$23.next())$jscomp$key$value=$jscomp$key$value.value,null!=($jscomp$key$value=valueof($jscomp$key$value,++index,values))&&(max<$jscomp$key$value||void 0===max&&$jscomp$key$value>=$jscomp$key$value)&&(max=$jscomp$key$value,maxIndex=index)}return maxIndex}function flatten(arrays){var $jscomp$iter$26,$jscomp$key$array,array$115;return $jscomp.generator.createGenerator(flatten,function($jscomp$generator$context){1==
$jscomp$generator$context.nextAddress&&($jscomp$iter$26=$jscomp.makeIterator(arrays),$jscomp$key$array=$jscomp$iter$26.next());if(3!=$jscomp$generator$context.nextAddress){if($jscomp$key$array.done)return $jscomp$generator$context.jumpTo(0);array$115=$jscomp$key$array.value;return $jscomp$generator$context.yieldAll(array$115,3)}$jscomp$key$array=$jscomp$iter$26.next();return $jscomp$generator$context.jumpTo(2)})}function minIndex(values,valueof){var minIndex=-1,index=-1;if(void 0===valueof){values=
$jscomp.makeIterator(values);for(var $jscomp$key$value=values.next();!$jscomp$key$value.done;$jscomp$key$value=values.next())if(valueof=$jscomp$key$value.value,++index,null!=valueof&&(min>valueof||void 0===min&&valueof>=valueof)){var min=valueof;minIndex=index}}else{var $jscomp$iter$28=$jscomp.makeIterator(values);for($jscomp$key$value=$jscomp$iter$28.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$28.next())$jscomp$key$value=$jscomp$key$value.value,null!=($jscomp$key$value=valueof($jscomp$key$value,
...
++index,0>min?0===compare($jscomp$key$value,$jscomp$key$value):0>compare($jscomp$key$value,minValue)){var minValue=$jscomp$key$value;min=index}return min}function shuffler(random){return function(array,i0,i1){i0=void 0===i0?0:i0;i1=void 0===i1?array.length:i1;for(i1-=i0=+i0;i1;){var i=random()*i1--|0,t$119=array[i1+i0];array[i1+i0]=array[i+i0];array[i+i0]=t$119}return array}}function transpose$jscomp$0(matrix){if(!(n=matrix.length))return[];for(var i=-1,m=min$jscomp$0(matrix,length$1),transpose=Array(m);++i<
m;)for(var j=-1,n,row=transpose[i]=Array(n);++j<n;)row[j]=matrix[j][i];return transpose}function length$1(d){return d.length}function set(values){return values instanceof Set?values:new Set(values)}function superset(values,other){var iterator=values[Symbol.iterator]();values=new Set;var $jscomp$loop$145={};other=$jscomp.makeIterator(other);for(var $jscomp$key$o=other.next();!$jscomp$key$o.done;$jscomp$loop$145={$jscomp$loop$prop$value$146:$jscomp$loop$145.$jscomp$loop$prop$value$146,$jscomp$loop$prop$done$147:$jscomp$loop$145.$jscomp$loop$prop$done$147},
$jscomp$key$o=other.next())if($jscomp$key$o=$jscomp$key$o.value,!values.has($jscomp$key$o))for($jscomp$loop$145.$jscomp$loop$prop$value$146=void 0,$jscomp$loop$145.$jscomp$loop$prop$done$147=void 0;function($jscomp$loop$145){return function(){var $jscomp$destructuring$var34=iterator.next();$jscomp$loop$145.$jscomp$loop$prop$value$146=$jscomp$destructuring$var34.value;$jscomp$loop$145.$jscomp$loop$prop$done$147=$jscomp$destructuring$var34.done;return $jscomp$destructuring$var34}}($jscomp$loop$145)();){if($jscomp$loop$145.$jscomp$loop$prop$done$147)return!1;
values.add($jscomp$loop$145.$jscomp$loop$prop$value$146);if(Object.is($jscomp$key$o,$jscomp$loop$145.$jscomp$loop$prop$value$146))break}return!0}var ascendingBisect=bisector(ascending),bisectRight=ascendingBisect.right;ascendingBisect=ascendingBisect.left;var bisectCenter=bisector(number).center,Adder=function(){this._partials=new Float64Array(32);this._n=0};Adder.prototype.add=function(x){for(var p=this._partials,i=0,j=0;j<this._n&&32>j;j++){var y=p[j],hi=x+y;(x=Math.abs(x)<Math.abs(y)?x-(hi-y):
...
_key:{value:key}});entries=$jscomp.makeIterator(entries);for(key=entries.next();!key.done;key=entries.next()){var $jscomp$destructuring$var7=$jscomp.makeIterator(key.value);key=$jscomp$destructuring$var7.next().value;$jscomp$destructuring$var7=$jscomp$destructuring$var7.next().value;$jscomp$super$this.set(key,$jscomp$destructuring$var7)}return $jscomp$super$this};$jscomp.inherits(InternMap,Map);InternMap.prototype.get=function(key){return Map.prototype.get.call(this,intern_get(this,key))};InternMap.prototype.has=
...
arguments.length;++$jscomp$restIndex)$jscomp$restParams[$jscomp$restIndex-0]=arguments[$jscomp$restIndex];var values$84=$jscomp$restParams;$jscomp$restParams="function"===typeof values$84[values$84.length-1]&&reducer$jscomp$0(values$84.pop());values$84=values$84.map(arrayify);$jscomp$restIndex=values$84.map(length$jscomp$0);var j$jscomp$0=values$84.length-1,index$86=Array(j$jscomp$0+1).fill(0),product=[];if(0>j$jscomp$0||$jscomp$restIndex.some(empty))return product;for(;;){product.push(index$86.map(function(j,
i){return values$84[i][j]}));for(var i$jscomp$0=j$jscomp$0;++index$86[i$jscomp$0]===$jscomp$restIndex[i$jscomp$0];){if(0===i$jscomp$0)return $jscomp$restParams?product.map($jscomp$restParams):product;index$86[i$jscomp$0--]=0}}};exports.cumsum=function(values,valueof){var sum=0,index=0;return Float64Array.from(values,void 0===valueof?function(v){return sum+=+v||0}:function(v){return sum+=+valueof(v,index++,values)||0})};exports.descending=function(a,b){return b<a?-1:b>a?1:b>=a?0:NaN};exports.deviation=
...
$jscomp$restIndex.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$restIndex.next())values.delete($jscomp$key$value.value)}return values};exports.disjoint=function(values,other){var iterator=other[Symbol.iterator]();other=new Set;var $jscomp$loop$142={};values=$jscomp.makeIterator(values);for(var $jscomp$key$v=values.next();!$jscomp$key$v.done;$jscomp$loop$142={$jscomp$loop$prop$value$143:$jscomp$loop$142.$jscomp$loop$prop$value$143,$jscomp$loop$prop$done$144:$jscomp$loop$142.$jscomp$loop$prop$done$144},
$jscomp$key$v=values.next()){$jscomp$key$v=$jscomp$key$v.value;if(other.has($jscomp$key$v))return!1;$jscomp$loop$142.$jscomp$loop$prop$value$143=void 0;for($jscomp$loop$142.$jscomp$loop$prop$done$144=void 0;function($jscomp$loop$142){return function(){var $jscomp$destructuring$var32=iterator.next();$jscomp$loop$142.$jscomp$loop$prop$value$143=$jscomp$destructuring$var32.value;$jscomp$loop$142.$jscomp$loop$prop$done$144=$jscomp$destructuring$var32.done;return $jscomp$destructuring$var32}}($jscomp$loop$142)()&&
!$jscomp$loop$142.$jscomp$loop$prop$done$144;){if(Object.is($jscomp$key$v,$jscomp$loop$142.$jscomp$loop$prop$value$143))return!1;other.add($jscomp$loop$142.$jscomp$loop$prop$value$143)}}return!0};exports.every=function(values,test){if("function"!==typeof test)throw new TypeError("test is not a function");for(var index=-1,$jscomp$iter$38=$jscomp.makeIterator(values),$jscomp$key$value=$jscomp$iter$38.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$38.next())if(!test($jscomp$key$value.value,
++index,values))return!1;return!0};exports.extent=extent;exports.filter=function(values,test){if("function"!==typeof test)throw new TypeError("test is not a function");for(var array=[],index=-1,$jscomp$iter$40=$jscomp.makeIterator(values),$jscomp$key$value=$jscomp$iter$40.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$40.next())$jscomp$key$value=$jscomp$key$value.value,test($jscomp$key$value,++index,values)&&array.push($jscomp$key$value);return array};exports.fsum=function(values,valueof){var adder=
new Adder;if(void 0===valueof){values=$jscomp.makeIterator(values);for(var $jscomp$key$value=values.next();!$jscomp$key$value.done;$jscomp$key$value=values.next())valueof=$jscomp$key$value.value,(valueof=+valueof)&&adder.add(valueof)}else{var index$91=-1,$jscomp$iter$12=$jscomp.makeIterator(values);for($jscomp$key$value=$jscomp$iter$12.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$12.next())$jscomp$key$value=$jscomp$key$value.value,($jscomp$key$value=+valueof($jscomp$key$value,++index$91,
...
values.next())if($jscomp$key$value=$jscomp$key$value.value,++index,0>max?0===compare($jscomp$key$value,$jscomp$key$value):0<compare($jscomp$key$value,maxValue)){var maxValue=$jscomp$key$value;max=index}return max};exports.group=group;exports.groupSort=function(values,reduce,key){return(1===reduce.length?sort(rollup(values,reduce,key),function($jscomp$destructuring$var18,$jscomp$destructuring$var19){var $jscomp$destructuring$var20=$jscomp.makeIterator($jscomp$destructuring$var18);$jscomp$destructuring$var18=
$jscomp$destructuring$var20.next().value;$jscomp$destructuring$var20=$jscomp$destructuring$var20.next().value;var $jscomp$destructuring$var21=$jscomp.makeIterator($jscomp$destructuring$var19);$jscomp$destructuring$var19=$jscomp$destructuring$var21.next().value;$jscomp$destructuring$var21=$jscomp$destructuring$var21.next().value;return ascending($jscomp$destructuring$var20,$jscomp$destructuring$var21)||ascending($jscomp$destructuring$var18,$jscomp$destructuring$var19)}):sort(group(values,key),function($jscomp$destructuring$var22,
$jscomp$destructuring$var23){var $jscomp$destructuring$var24=$jscomp.makeIterator($jscomp$destructuring$var22);$jscomp$destructuring$var22=$jscomp$destructuring$var24.next().value;$jscomp$destructuring$var24=$jscomp$destructuring$var24.next().value;var $jscomp$destructuring$var25=$jscomp.makeIterator($jscomp$destructuring$var23);$jscomp$destructuring$var23=$jscomp$destructuring$var25.next().value;$jscomp$destructuring$var25=$jscomp$destructuring$var25.next().value;return reduce($jscomp$destructuring$var24,
$jscomp$destructuring$var25)||ascending($jscomp$destructuring$var22,$jscomp$destructuring$var23)})).map(function($jscomp$destructuring$var26){return $jscomp.makeIterator($jscomp$destructuring$var26).next().value})};exports.groups=function(values,keys){for(var $jscomp$restParams=[],$jscomp$restIndex=1;$jscomp$restIndex<arguments.length;++$jscomp$restIndex)$jscomp$restParams[$jscomp$restIndex-1]=arguments[$jscomp$restIndex];return nest(values,Array.from,identity,$jscomp$restParams)};exports.histogram=
...
$jscomp$key$value.value;for(var $jscomp$iter$44=$jscomp.makeIterator($jscomp$restParams),$jscomp$key$other=$jscomp$iter$44.next();!$jscomp$key$other.done;$jscomp$key$other=$jscomp$iter$44.next())if(!$jscomp$key$other.value.has($jscomp$key$value)){values.delete($jscomp$key$value);continue a}}return values};exports.least=function(values,compare){compare=void 0===compare?ascending:compare;var defined=!1;if(1===compare.length){values=$jscomp.makeIterator(values);for(var $jscomp$key$element=values.next();!$jscomp$key$element.done;$jscomp$key$element=
...
values.next())valueof=$jscomp$key$value.value,null!=valueof&&(valueof=+valueof)>=valueof&&(++count,sum+=valueof)}else{var index$113=-1,$jscomp$iter$25=$jscomp.makeIterator(values);for($jscomp$key$value=$jscomp$iter$25.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$25.next())$jscomp$key$value=$jscomp$key$value.value,null!=($jscomp$key$value=valueof($jscomp$key$value,++index$113,values))&&($jscomp$key$value=+$jscomp$key$value)>=$jscomp$key$value&&(++count,sum+=$jscomp$key$value)}if(count)return sum/
...
typeof test)throw new TypeError("test is not a function");for(var index=-1,$jscomp$iter$39=$jscomp.makeIterator(values),$jscomp$key$value=$jscomp$iter$39.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$39.next())if(test($jscomp$key$value.value,++index,values))return!0;return!1};exports.sort=sort;exports.subset=function(values,other){return superset(other,values)};exports.sum=function(values,valueof){var sum=0;if(void 0===valueof){values=$jscomp.makeIterator(values);for(var $jscomp$key$value=
values.next();!$jscomp$key$value.done;$jscomp$key$value=values.next())if(valueof=$jscomp$key$value.value,valueof=+valueof)sum+=valueof}else{var index$120=-1,$jscomp$iter$37=$jscomp.makeIterator(values);for($jscomp$key$value=$jscomp$iter$37.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$37.next())if($jscomp$key$value=$jscomp$key$value.value,$jscomp$key$value=+valueof($jscomp$key$value,++index$120,values))sum+=$jscomp$key$value}return sum};exports.superset=superset;exports.thresholdFreedmanDiaconis=

...
scope_1=require("module$node_modules$$sentry$hub$dist$scope"),session_1=require("module$node_modules$$sentry$hub$dist$session");exports.API_VERSION=3;var Hub=function(){function Hub(client,scope,_version){void 0===scope&&(scope=new scope_1.Scope);void 0===_version&&(_version=exports.API_VERSION);this._version=_version;this._stack=[{}];this.getStackTop().scope=scope;this.bindClient(client)}Hub.prototype.isOlderThan=function(version){return this._version<version};Hub.prototype.bindClient=function(client){(this.getStackTop().client=
client)&&client.setupIntegrations&&client.setupIntegrations()};Hub.prototype.pushScope=function(){var scope=scope_1.Scope.clone(this.getScope());this.getStack().push({client:this.getClient(),scope:scope});return scope};Hub.prototype.popScope=function(){return 1>=this.getStack().length?!1:!!this.getStack().pop()};Hub.prototype.withScope=function(callback){var scope=this.pushScope();try{callback(scope)}finally{this.popScope()}};Hub.prototype.getClient=function(){return this.getStackTop().client};Hub.prototype.getScope=
function(){return this.getStackTop().scope};Hub.prototype.getStack=function(){return this._stack};Hub.prototype.getStackTop=function(){return this._stack[this._stack.length-1]};Hub.prototype.captureException=function(exception,hint){var eventId=this._lastEventId=utils_1.uuid4(),finalHint=hint;if(!hint){try{throw Error("Sentry syntheticException");}catch(exception$69){hint=exception$69}finalHint={originalException:exception,syntheticException:hint}}this._invokeClient("captureException",exception,tslib_1.__assign(tslib_1.__assign({},
...
context,customSamplingContext)};Hub.prototype.traceHeaders=function(){return this._callExtensionMethod("traceHeaders")};Hub.prototype.startSession=function(context){this.endSession();var _a=this.getStackTop(),scope=_a.scope;_a=(_a=_a.client)&&_a.getOptions()||{};context=new session_1.Session(tslib_1.__assign(tslib_1.__assign({release:_a.release,environment:_a.environment},scope&&{user:scope.getUser()}),context));scope&&scope.setSession(context);return context};Hub.prototype.endSession=function(){var _a=
this.getStackTop(),scope=_a.scope;_a=_a.client;if(scope){var session=scope.getSession();session&&(session.close(),_a&&_a.captureSession&&_a.captureSession(session),scope.setSession())}};Hub.prototype._invokeClient=function(method){for(var args=[],_i=1;_i<arguments.length;_i++)args[_i-1]=arguments[_i];var _b=this.getStackTop();_i=_b.scope;(_b=_b.client)&&_b[method]&&_b[method].apply(_b,tslib_1.__spread(args,[_i]))};Hub.prototype._callExtensionMethod=function(method){for(var args=[],_i=1;_i<arguments.length;_i++)args[_i-
...
setHubOnCarrier(registry,new Hub);if(utils_1.isNodeEnv()){try{var activeDomain=getActiveDomain();if(activeDomain){if(!(activeDomain&&activeDomain.__SENTRY__&&activeDomain.__SENTRY__.hub)||getHubFromCarrier(activeDomain).isOlderThan(exports.API_VERSION)){var registryHubTopStack=getHubFromCarrier(registry).getStackTop();setHubOnCarrier(activeDomain,new Hub(registryHubTopStack.client,scope_1.Scope.clone(registryHubTopStack.scope)))}var JSCompiler_inline_result=getHubFromCarrier(activeDomain)}else JSCompiler_inline_result=
getHubFromCarrier(registry)}catch(_Oo){JSCompiler_inline_result=getHubFromCarrier(registry)}return JSCompiler_inline_result}return getHubFromCarrier(registry)};exports.getActiveDomain=getActiveDomain;exports.getHubFromCarrier=getHubFromCarrier;exports.setHubOnCarrier=setHubOnCarrier}
